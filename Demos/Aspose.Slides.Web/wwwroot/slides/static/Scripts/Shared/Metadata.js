(function ($) {
    $.displayMetadata = function (data, onSave, onClearAll, onReset) {
        const BUILT_IN = 1;
        const CUSTOM = 2;
        ...
        var modify = function (id) {
            ...
            if (tr.length > 0) {
                ...
                if (tdValue.length && tdButton.length && tdType.length) {
                    ...
                    var btnUpdate = $('<input type="image" src="/slides/static/img/grid-update-ico.png" title="Update" alt="Update" style="border-width:0" />');
                    btnUpdate.on('click', onClickUpdate);
                    tdButton.append(btnUpdate);
                    tdButton.append('&nbsp;');

                    var btnCancel = $('<input type="image" src="/slides/static/img/grid-cancel-ico.png" title="Cancel" alt="Cancel" style="border-width:0" />');
        ...
        var display = function () {
            ...
            for (var i in items) {
                ...
                var btn = $('<input type="image" src="/slides/static/img/grid-edit-ico.png" alt="Edit" title="Edit" style="border-width:0" data-target="' + items[i].Id + '" />');
                btn.on('click', onClickModify);
                td.append(btn);
                if (mode === CUSTOM) {
                    btn = $('<input type="image" src="/slides/static/img/grid-delete-ico.png" alt="Delete" title="Delete" style="border-width:0" data-target="' + items[i].Id + '" />');
