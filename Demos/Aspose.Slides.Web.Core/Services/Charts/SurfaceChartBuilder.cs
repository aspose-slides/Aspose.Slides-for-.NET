...
{
	internal sealed class SurfaceChartBuilder : BaseChartBuilder
	{
		public SurfaceChartBuilder(ChartType chartType) : base(chartType)
		{
		}
			...
			for (var rowIndex = 0; rowIndex < rowCount; rowIndex++)
			{
				...
				for (var colIndex = 0; colIndex < colCount; colIndex++)
				{
					...
					if (cell.Type == CellValueType.IsNumeric)
					{
						...
						series.DataPoints.AddDataPointForSurfaceSeries(fact.GetCell(defaultWorksheetIndex, dataRow, dataColumn, cell.Value));
					}
				}
			}
		}
