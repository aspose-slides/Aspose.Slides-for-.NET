...
{
	internal sealed class DoughnutChartBuilder : BaseChartBuilder
	{
		private const float DefaultDoughnutChartStartPointX = 0f;
		private const float DefaultDoughnutChartStartPointY = 25f;

		public DoughnutChartBuilder(ChartType chartType) : base(chartType)
		{
		}
		...
		protected override IChart GetChart(ISlide slide)
		{
			...
			return slide.Shapes.AddChart(_chartType, DefaultDoughnutChartStartPointX, DefaultDoughnutChartStartPointY, chartWidth, chartHeight);
		}
			...
			for (var rowIndex = 0; rowIndex < rowCount; rowIndex++)
			{
				...
				for (var colIndex = 0; colIndex < colCount; colIndex++)
				{
					...
					if (cell.Type == CellValueType.IsNumeric)
					{
						...
						series.DataPoints.AddDataPointForDoughnutSeries(fact.GetCell(defaultWorksheetIndex, dataRow, dataColumn, cell.Value));						
					}
				}
			}
