...
{
	internal sealed class WaterfallChartBuilder : BaseChartBuilder
	{
		private const float DefaultWaterfallChartStartPointX = 0f;
		private const float DefaultWaterfallChartStartPointY = 0f;

		public WaterfallChartBuilder(ChartType chartType) : base(chartType)
		{
		}
		...
		protected override IChart GetChart(ISlide slide)
		{
			...
			return slide.Shapes.AddChart(_chartType, DefaultWaterfallChartStartPointX, DefaultWaterfallChartStartPointY, chartWidth, chartHeight);
		}
			...
			for (var rowIndex = 0; rowIndex < rowCount; rowIndex++)
			{
				...
				for (var colIndex = 0; colIndex < colCount; colIndex++)
				{
					...
					if (rowIndex == 0)
					{
						...
						chart.ChartData.Series.Add(fact.GetCell(defaultWorksheetIndex, rowIndex, colIndex + columnShift + indexShift, colTitle), _chartType);						
					}
					...
					if (cell.Type == CellValueType.IsNumeric)
					{
						...
						series.DataPoints.AddDataPointForWaterfallSeries(fact.GetCell(defaultWorksheetIndex, dataRow, dataColumn, cell.Value));
					}
				}
			}
		}
