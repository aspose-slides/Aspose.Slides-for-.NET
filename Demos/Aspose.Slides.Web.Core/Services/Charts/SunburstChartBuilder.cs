...
{
	internal sealed class SunburstChartBuilder : BaseChartBuilder
	{
		private const float DefaultSunburstChartStartPointX = 0f;
		private const float DefaultSunburstChartStartPointY = 0f;

		public SunburstChartBuilder(ChartType chartType) : base(chartType)
		{
		}
		...
		protected override IChart GetChart(ISlide slide)
		{
			...
			return slide.Shapes.AddChart(_chartType, DefaultSunburstChartStartPointX, DefaultSunburstChartStartPointY, chartWidth, chartHeight);
		}
				...
				if (!branches.Contains(cellBranch.StringValue))
				{
					...
					if (!stems.Contains(cellStem.StringValue))
					{
						...
						if (!leaves.Contains(cellLeaf.StringValue))
						{
							...
							series.DataPoints.AddDataPointForSunburstSeries(fact.GetCell(defaultWorksheetIndex, rowIndex, 3, cellData.DoubleValue));
