...
{
	internal sealed class ParetoLineChartBuilder : BaseChartBuilder
	{
		private const float DefaultParetoLineChartStartPointX = 0f;
		private const float DefaultParetoLineChartStartPointY = 0f;

		public ParetoLineChartBuilder(ChartType chartType) : base(chartType)
		{
		}
		...
		protected override IChart GetChart(ISlide slide)
		{
			...
			// The ChartType.ParetoLine is ordinary ChartType.Histogram + chart.ChartData.Series.Add(ChartType.ParetoLine);
			return slide.Shapes.AddChart(ChartType.Histogram, DefaultParetoLineChartStartPointX, DefaultParetoLineChartStartPointY, chartWidth, chartHeight, false);
		}
			...
			for (var rowIndex = 0; rowIndex < rowCount; rowIndex++)
			{
				...
				for (var colIndex = 0; colIndex < colCount; colIndex++)
				{
					...
					if (colIndex == 0)
					{
						...
						chart.ChartData.Series.Add(fact.GetCell(defaultWorksheetIndex, rowIndex + rowShift + indexShift, colIndex, rowTitle), chart.Type);
					}
					...
					if (cell.Type == CellValueType.IsNumeric)
					{
						...
						series.DataPoints.AddDataPointForHistogramSeries(fact.GetCell(defaultWorksheetIndex, dataRow, dataColumn, cell.Value));
					}
				}
			}

			chart.ChartData.Series.Add(_chartType);
		}
	}
}
