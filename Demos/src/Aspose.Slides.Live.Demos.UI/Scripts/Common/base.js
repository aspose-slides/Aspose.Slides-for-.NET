			...
			if (o.ShowViewerButton) {
				...
				if (ext !== null && viewerlink.length && VIEWABLE_EXTENSIONS.indexOf(ext) !== -1) {
					viewerlink.on('click', function (evt) {
						evt.preventDefault();
						evt.stopPropagation();
						openIframe(generateViewerLink(data), '/slides/viewer', '/slides/view');
					});
				}
				else {
					viewerlink.hide();
...
function requestMetadata(data) {
	...
	if (response.length > 0) {
		let url = o.UIBasePath + 'api/AsposeSlidesMetadata/properties?folderName=' + response[2] + "&fileName=" + response[1];
		$.ajax({
			type: 'POST',
			url: url,			
...
function requestProtect() {
	...
	if (data === null)
		return;
	let url = o.UIBasePath + 'Lock/Lock?passw=' + encodeURI($('#passw').val()) + '&viewpassw=' + encodeURI($('#viewpassword').val()) + "&makefinal=" + $('#makefinal').prop("checked");
...
function requestViewer(data) {
	...
	openIframe(url, '/slides/viewer', '/slides/view');
}
