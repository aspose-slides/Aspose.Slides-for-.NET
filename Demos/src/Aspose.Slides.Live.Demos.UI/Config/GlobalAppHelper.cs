...
namespace Aspose.Slides.Live.Demos.UI.Config
{
    ...
    public class GlobalAppHelper
    {
		...
		public GlobalAppHelper(HttpContext hc, HttpApplicationState appState, string SessionID, string language)
		{
			...
			AsposeSlidesContext.atcc = new AsposeSlidesContext(hc); // sync the context
		...
		private void initResources(string ResourcesFile, string SessionID)
		{
			...
			if (AsposeSlidesContext.Current.Cache[SessionID] == null)
			{
				...
				AsposeSlidesContext.Current.Cache.Add(
				   SessionID,
					resources,
