...
namespace Aspose.Slides.Live.Demos.UI.Config
{
	public class BasePage : BaseRootPage
	{
		...
		protected void PerformResponse(FileSafeResult response, HtmlGenericControl controlErrorMessage, Action<FileSafeResult> action)
		{
			if (response == null)
		...
		/// <param name="response"></param>
		protected void ShowErrorMessage(HtmlGenericControl control, FileSafeResult response)
		{
			string txt;
			txt = "Failed!";
		...
		protected Collection<FileUploadResponse> UploadFiles ( string _folderName, params HtmlInputFile[] fileInputs )
		{
